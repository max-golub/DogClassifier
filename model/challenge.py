...
class Challenge(nn.Module):
    def __init__(self):
        ...
        self.fco1 = nn.Linear(in_features=32,out_features=64)
        self.fco2 = nn.Linear(in_features=64, out_features=2)
        ##
    ...
    def init_weights(self):
        ...
        Cin = self.fco1.weight.size(1)
        nn.init.normal_(self.fco1.weight, 0.0, 1 / sqrt(Cin))
        nn.init.constant_(self.fco1.bias, 0.0)
        
        Cin = self.fco2.weight.size(1)
        nn.init.normal_(self.fco2.weight, 0.0, 1 / sqrt(Cin))
        nn.init.constant_(self.fco2.bias, 0.0)
        ##
